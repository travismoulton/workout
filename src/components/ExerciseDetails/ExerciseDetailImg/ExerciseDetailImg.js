...
const ExerciseDetailImg = ({ primaryMuscles, secondaryMuscles }) => {
  ...
  */
  ...
  const primaryFrontMusclesStr = primaryMuscles
    .map((muscle) =>
      wgerDict.muscles[muscle].is_front
        ? `URL(${wgerDict.muscles[muscle].image_url_main})`
        : ''
    )
    .filter((str) => str !== '')
    .join(',');
  ...
  const secondaryMusclesFrontStr = secondaryMuscles
    .map((muscle) =>
      wgerDict.muscles[muscle].is_front
        ? `URL(${wgerDict.muscles[muscle].image_url_secondary})`
        : ''
    )
    .filter((str) => str !== '')
    .join(',');
...
export default ExerciseDetailImg;
